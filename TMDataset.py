...
class TMDataset:
    ...
    def get_header(self):        
        if len(self.header_with_features) == 0:
            self.__fill_data_structure(self.add_mean, self.add_max, self.add_min, self.add_std, self.add_kurtosis, self.add_skewness)
    ...
    def __fill_data_structure(self, add_mean=True, add_max=True, add_min=True, add_std=True, add_kurtosis=False, add_skewness=False):
        if not self.sintetic:
        ...
        for s in self.sensors:
            if s != "activityrecognition":
                if add_mean:
                    ...
                    header_index += 1
                if add_min:
                    self.header_with_features[header_index] = s + "#min"
                    header_index += 1
                if add_max:
                    self.header_with_features[header_index] = s + "#max"    
                    header_index += 1
                if add_std:
                    self.header_with_features[header_index] = s + "#std"
                    header_index += 1
                if add_skewness:
                    self.header_with_features[header_index] = s + "#skewness"
                    header_index += 1
                if add_kurtosis:
                    self.header_with_features[header_index] = s + "#kurtosis"
                    header_index += 1
    ...
    def __range_position_in_header_with_features(self, sensor_name, add_mean=True, add_max=True, add_min=True, add_std=True, add_kurtosis=False, add_skewness=False):
        if len(self.header) == 0 or len(self.header_with_features) == 0:
            self.__fill_data_structure(add_mean, add_max, add_min, add_std, add_kurtosis, add_skewness)
        range_position = []
        start_pos = end_pos = -1
        i = 0
        found = False
    ...
    def create_header_files(self, add_mean=True, add_max=True, add_min=True, add_std=True, add_kurtosis=False, add_skewness=False):
        if self.sintetic:
            dir_src = const.DIR_SINTETIC_RAW_DATA_TRANSFORM
    ...
    def __create_time_files(self, add_mean=True, add_max=True, add_min=True, add_std=True, add_kurtosis=False, add_skewness=False):
        if self.sintetic:
        ...
        if not os.path.exists(dir_src):
            self.create_header_files(add_mean, add_max, add_min, add_std, add_kurtosis, add_skewness)
        if len(self.header) == 0 or len(self.header_with_features) == 0:
            self.__fill_data_structure(add_mean, add_max, add_min, add_std, add_kurtosis, add_skewness)
        if not os.path.exists(dir_dst):
            os.makedirs(dir_dst)
            ...
            if current_file.endswith("csv"):
                ...
                while True:
                    ...
                    for feature in featureNames:
                        ...
                        currentSkewness = currentFeatureSerie.skew(skipna=True)
                        currentKurtosis = currentFeatureSerie.kurtosis(skipna=True)
                        if i == 0:
                            ...
                            current_skewness.append(str(currentSkewness))
                            current_kurtosis.append(str(currentKurtosis))
                        else:
                            ...
                            if str(currentStd) == 'nan':
                                ...
                                current_std.append(str(currentStd))
                            if str(currentSkewness) == 'nan':
                                current_skewness.append(str(previous_skewness[nfeature]))
                            else:
                                current_skewness.append(str(currentSkewness))
                            if str(currentKurtosis) == 'nan':
                                current_kurtosis.append(str(previous_kurtosis[nfeature]))
                            else:
                                current_kurtosis.append(str(currentKurtosis))
                            ...
                            if add_skewness:
                                currentLine = currentLine + str(current_skewness[nfeature]) + ","
                            if add_kurtosis:
                                currentLine = currentLine + str(current_kurtosis[nfeature]) + ","
                            nfeature += 1
                    ...
                    previous_skewness = list(current_skewness)
                    previous_kurtosis = list(current_kurtosis)
    ...
    # create dataset file
    def __create_dataset(self, add_mean=True, add_max=True, add_min=True, add_std=True, add_kurtosis=False, add_skewness=False):
        if self.sintetic:
        ...
        if not os.path.exists(dir_src):
            self.__create_time_files(add_mean, add_max, add_min, add_std, add_kurtosis, add_skewness)
    ...
    def create_balanced_dataset(self, sintetic = False, add_mean=True, add_max=True, add_min=True, add_std=True, add_kurtosis=False, add_skewness=False):
        ...
        self.__create_dataset(add_mean, add_max, add_min, add_std, add_kurtosis, add_skewness)
